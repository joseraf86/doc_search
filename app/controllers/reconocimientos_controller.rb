class ReconocimientosController < AdminController

  # GET /reconocimientos
  # GET /reconocimientos.json
  def index
    @reconocimientos = Reconocimiento.order("descripcion").paginate(:page => params[:page])
  ...
  def update
    ...
    respond_to do |format|
      if @reconocimiento.update_attributes(params[:reconocimiento])
        format.html { redirect_to @reconocimiento, notice: 'Reconocimiento actualizado' }
